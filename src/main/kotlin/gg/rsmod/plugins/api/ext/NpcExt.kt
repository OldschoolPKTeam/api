...
/**
 * Drains an [Npc]s skill and returns the amount that was drained
 ...
 */
fun Npc.drainSkill(skillId: Int, drainPercentage: Double, capped: Boolean = true): Int {
    val currentLevel = this.stats.getCurrentLevel(skillId)
    val drainAmount = currentLevel * drainPercentage
    this.stats.decrementCurrentLevel(skillId, drainAmount.toInt(), capped)
    return currentLevel - this.stats.getCurrentLevel(skillId)
}
 ...
 * @param capped Determines whether the skill should drain based on the max level
 */
fun Npc.drainSkill(skillId: Int, drainAmount: Int, capped: Boolean = true): Int {
    val currentLevel = this.stats.getCurrentLevel(skillId)
    this.stats.decrementCurrentLevel(skillId, drainAmount, capped)
    return currentLevel - this.stats.getCurrentLevel(skillId)
}